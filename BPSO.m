%This file is intened to perform theBianry Particle Swarm Genetic search Algorithm
%in proposed algorithm structure
%The change of Ant_Size and genertion_n would impact on its performance

function [best,xGbest]=BPSO(fhd,uv,yv,D,var_n,K,mut_rate)
tt=100;	% Times of moving
...
for i = 1:tt
    ...
    if size(Multiple,2)>1
        ...
        index3=find(fitness==best,1,'first');
    end
    second=max(fitness(fitness<max(fitness)));
    Multiple=find(fitness==second);
    if size(Multiple,2)>1
        ...
        index4=find(fitness==second,1,'first');
    end
    % Update xGbest and restore best
    Elite_popu([1 2], :) = popu([index3 index4], :);
    if best>GlobalBest
        GlobalBest=best;
        xGbest=popu(index3,:);
    end
